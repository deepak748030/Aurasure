 ...
 *   - `__DEV__` (local Metro / debug builds) is skipped so the sheet never pops
...
async function readDismissedVersion(): Promise<string | null> {
  return readJson<string | null>(DISMISSED_KEY, null);
}

async function writeDismissedVersion(version: string): Promise<void> {
  await writeJson(DISMISSED_KEY, version);
}
 ...
 * app can render its own in-app prompt — the user updates WITHOUT leaving the
 * app. Update type follows the Play Console priority: priority ≥ 4 uses the
 * full-screen immediate flow, anything else downloads in the background.
 */
export function useInAppUpdates(): UseInAppUpdates {
  ...
  const loadModule = useCallback(async (): Promise<UpdateModule | null> => {
    ...
    try {
      ...
      // The native module is missing (e.g. Expo Go) — the gate stays idle.
      return null;
    }
  }, []);
