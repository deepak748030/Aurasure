...
export function LoginScreen({ navigation }: Props): React.ReactElement {
  ...
  const submit = async () => {
    ...
    setError('');
    try {
      const data = await riderLogin(phone.trim(), password);
      haptic.success();
      setRider(data.rider);
    } catch (err) {
      haptic.error();
  ...
  return (
    <Screen
      ...
      title="Rider sign in"
      subtitle="The number you registered as a partner"
    >
      <View style={{ alignItems: 'center', marginBottom: 18, marginTop: 8 }}>
        ...
        >
          <Icon name="bike" size={32} color="#9C005E" />
        </View>
      </View>
      <Input
        label="Mobile"
      ...
      <Button title="Create a rider account" variant="ghost" onPress={() => navigation.navigate('Register')} />
    </Screen>
  );
}
