...
export function MapScreen({ navigation }: Props): React.ReactElement { const { vendor, setVendor } = useVendor(); const { height } = useWindowDimensions(); const [point, setPoint] = useState({ lat: vendor?.geo?.lat ?? 22.7196, lng: vendor?.geo?.lng ?? 75.8577 }); const [busy, setBusy] = useState(false); const save = async () => { if (!vendor?.outletId) return; setBusy(true); try { const result = await vendorApi.updateOutlet(vendor.outletId, { geo: point }); setVendor(result.vendor); Alert.alert('Pin saved', 'Dispatch will use this location for rider pickup.'); navigation.goBack(); } catch (e) { Alert.alert('Could not save pin', e instanceof Error ? e.message : 'Try again'); } finally { setBusy(false); } };
