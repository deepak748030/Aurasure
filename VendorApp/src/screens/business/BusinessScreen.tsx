...
export function BusinessScreen({ navigation }: Props): React.ReactElement {
  ...
  const openStatement = async (id: string) => { try { const result = await vendorApi.statement(id); showModal({ title: `Statement · ${String(result.statement.orderCode)}`, message: `Gross ${money(Number(result.statement.gross))}\nCommission ${money(Number(result.statement.commission))}\nNet ${money(Number(result.statement.net))}` }); } catch { showModal({ title: 'Statement unavailable', message: 'Try again later.' }); } };
  return (
    <Screen title="Business" subtitle="Understand the money behind every order" headerLeft={<BackButton onPress={() => navigation.goBack()} />} onRefresh={() => void load()}>
      ...
      <SectionTitle title="Settlement wallet" action="How it works" onAction={() => showModal({ title: 'Settlement', message: 'Net is calculated from delivered orders after the platform commission.' })} />
